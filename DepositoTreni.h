...
public:
	...
	int get_priorita(const int);						//sistema le priorità dei treni 0 superveloce, 1 veloce, 2 regionale
	bool isEmpty() const;								//controlla se ci sono treni nel deposito
	Train* elimina_treno();								//fa partire un treno dal deposito
	void print(const int) const;						//stampa i treni dello stesso tipo del deposito
